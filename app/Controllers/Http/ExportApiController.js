...
class ExportApiController {
  ...
  async providers({response}) {
    return response.api(200, Config.get('exportapis'));
  }
  ...
  async create ({ request, response, view }) {
    const rules = {
      name: 'required|string|unique:export_apis,name',
  ...
  async show ({ params, request, response, view }) {
    try {
      const template = await ExportApi.findOrFail(params.id);
  ...
  async update ({ params, request, response }) {
    const rules = {
      name: `required|string|unique:export_apis,name,id,${params.id}`,
      service: 'required|string',
      configuration: 'required',
    };
    const validation = await validate(request.all(), rules);
    ...
    try {
      const t = await ExportApi.findOrFail(params.id);
      t.name = request.input('name');
      t.service = request.input('service');
      t.configuration = request.input('configuration');
      await t.save();
...
module.exports = ExportApiController

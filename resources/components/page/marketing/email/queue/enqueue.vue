            ...
            <div v-if="!loading && !error">
                ...
                <div class="form-group mb-3">
                    <div class="input-group">
                        ...
                        <searchable-select class="custom-select" v-model="htmlMailMergeVal">
                            <option v-for="(type, key, index) in mapping" :key="index" :value="`{{${key}}}`">{{key}}</option>
                        </searchable-select>
                        <div class="input-group-append">
                            <button @click="addMailMergeValueToHTML()" class="btn btn-outline-success" type="button" id="button-addon2"><i class="icon-add"></i></button>
                        </div>
                    </div>
                </div>
                ...
                <csv-tags-field v-model="configuration.manual_recipients" label="Manually Specify Recipient Email Addresses" />
...
<script>
    ...
    export default {
        ...
        async mounted() {
            this.title = ('undefined' === typeof(this.$route.params.id)) ? 'Create an Email Blast' : `Edit Email Blast #${this.$route.params.id}`;
        ...
        methods: {
            ...
            async submitForm() {
                ...
                try {
                    ...
                    this.$router.push(`/marketing/email/blasts/${response.data.body.id}`);
                    swal.close();
                } catch (error) {
                    swal.close();

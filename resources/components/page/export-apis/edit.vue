...
<script>
    export default {
        ...
        async mounted() {
            this.title = ('undefined' === typeof(this.$route.params.id)) ? 'Create an Export API' : `Edit Export API #${this.$route.params.id}`;
        ...
        methods: {
            async submitForm() {
                swal({
                    title: 'Processing',
                    text: 'Saving the Export API. Please wait.',
                    type: 'info',
                ...
                const url = ('undefined' === typeof(this.$route.params.id)) ? '/export-apis/' : `/export-apis/${this.$route.params.id}`;
                ...
                try {
                    ...
                    await swal({
                        title: 'Success',
                        text: 'Saved Export API Successfully.',
                        type: 'success',
                        buttonsStyling: false,
                    ...
                    if ('undefined' === typeof(this.$route.params.id)) {
                        this.$router.push(`/export/apis/${response.data.body.id}`);
                    }
                    swal.close();
                } catch (error) {
                    swal.close();
            ...
            async loadApiInformation() {
                this.loading = true;
                const path = `/export-apis/${this.$route.params.id}`;
                try {
